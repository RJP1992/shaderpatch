...
namespace sp::effects {
            ...
            const Postprocess_options options) noexcept
        {
            ...
            if (fog_enabled) {
                work_texture = do_fog(dc, rt_allocator, input.depth_srv,
                    input.depth_srv_far,
                    input.projection_from_view,
                    input.projection_from_view_far,
                    input.view_matrix,
                    camera_position,
                    input.sun_direction,
                    input.sun_color,
    ...
    private:
        ...
        auto do_fog(ID3D11DeviceContext1& dc, Rendertarget_allocator& allocator,
            ...
            const glm::mat4& projection_from_view,
            const glm::mat4& projection_from_view_far,
            const glm::mat4& view_matrix,
            const glm::vec3& camera_position,
            const glm::vec3& map_sun_direction,
            const glm::vec3& map_sun_color,
            const core::Shader_resource_database& textures,
            float time,
            const Work_texture& input, Profiler& profiler) noexcept
            -> Work_texture
        {
            Profile profile{ profiler, dc, "Postprocessing - BF3 Atmospheric Fog" };

            // Compute inverse view-projection matrices for both depth buffers
            const glm::mat4 view_proj = projection_from_view * view_matrix;
            const glm::mat4 inv_view_proj = glm::inverse(view_proj);

            const glm::mat4 view_proj_far = projection_from_view_far * view_matrix;
            const glm::mat4 inv_view_proj_far = glm::inverse(view_proj_far);

            // Store captured projection values for debug display
            // m33 = [2][2], m43 = [3][2] in column-major (GLM)
            _fog_params.captured_near_m33 = projection_from_view[2][2];
            _fog_params.captured_near_m43 = projection_from_view[3][2];
            _fog_params.captured_far_m33 = projection_from_view_far[2][2];
            _fog_params.captured_far_m43 = projection_from_view_far[3][2];

            // Update fog constant buffer
            Fog_constants constants;
            constants.inv_view_proj = inv_view_proj;
            constants.inv_view_proj_far = inv_view_proj_far;
            constants.fog_color = _fog_params.color;
            constants.fog_density = _fog_params.density;
            constants.camera_position = camera_position;
            constants.fog_start = _fog_params.start_distance;
            constants.fog_end = _fog_params.end_distance;

            // Height fog
            constants.height_density = _fog_params.height_fog_enabled ? _fog_params.height_density : 0.0f;
            constants.height_base = _fog_params.height_base;
            constants.height_falloff = _fog_params.height_falloff;

            // Sun inscattering - uses map sun direction automatically
            if (_fog_params.sun_inscatter_enabled) {
                // Negate direction: light_directional_0_dir points FROM sun,
                // but inscatter needs direction TO sun
                constants.sun_direction = -map_sun_direction;
                constants.sun_intensity = _fog_params.sun_intensity;
                constants.sun_color = map_sun_color;
                constants.sun_power = _fog_params.sun_power;
            }
            else {
                constants.sun_direction = glm::vec3(0.0f, 1.0f, 0.0f);
                constants.sun_intensity = 0.0f;
                constants.sun_color = _fog_params.color;
                constants.sun_power = 8.0f;
            }

            constants.max_fog_opacity = _fog_params.max_opacity;
            ...
            // Cubemap mip blur for sun bleed-through prevention
            constants.cubemap_mip_scale = _fog_params.cubemap_mip_scale;

            // BF3 Atmosphere (two-stage: atmosphere cubemap + fog color overlay)
            if (_fog_params.atmosphere_enabled) {
                constants.atmos_intensity = _fog_params.atmos_intensity;
                constants.atmos_falloff = _fog_params.atmos_falloff;
                constants.horizon_offset = _fog_params.horizon_offset;
                constants.use_atmosphere_cubemap = !_fog_params.atmosphere_texture_name.empty() ? 1 : 0;

                // BF3 height blending: lerp(atmos, atmosdata.z, heightfactor * atmosdata.w)
                constants.atmos_high_intensity = _fog_params.atmos_high_intensity;
                constants.height_blend_weight = _fog_params.height_blend_weight;
            }
            else {
                constants.atmos_intensity = 0.0f;
                constants.atmos_falloff = 0.5f;
                constants.horizon_offset = 0.0f;
                constants.use_atmosphere_cubemap = 0;
                constants.atmos_high_intensity = 0.0f;
                constants.height_blend_weight = 0.0f;
            }

            // Cubemap alignment transform - build rotation matrix from Euler angles
            {
                const glm::vec3 rot_rad = glm::radians(_fog_params.cubemap_rotation);
                ...
                constants.cubemap_rotation_row0 = glm::vec4(rotation[0], 0.0f);
                constants.cubemap_rotation_row1 = glm::vec4(rotation[1], 0.0f);
                constants.cubemap_rotation_row2 = glm::vec4(rotation[2], 0.0f);

                constants.cubemap_scale = _fog_params.cubemap_scale;
                constants.cubemap_offset = _fog_params.cubemap_offset;
            }

            // Cloud integration
            constants.cloud_tint = _fog_params.cloud_tint;
            constants.cloud_boost = _fog_params.cloud_boost;

            // Zenith haze
            constants.zenith_haze = _fog_params.zenith_haze;

            // Space cubemap blending
            constants.altitude_blend_start = _fog_params.altitude_blend_start;
            constants.altitude_blend_end = _fog_params.altitude_blend_end;
            constants.sky_blend_override = _fog_params.sky_blend_override;
            constants.use_space_cubemap = !_fog_params.space_texture_name.empty() ? 1 : 0;

            // Debug depth linearization
            constants.debug_depth_enabled = _fog_params.debug_depth_enabled ? 1 : 0;
            constants.debug_buffer_mode = _fog_params.debug_buffer_mode;
            constants.debug_max_distance = _fog_params.debug_max_distance;
            constants.near_scene_near = _fog_params.near_scene_near;
            constants.near_scene_far = _fog_params.near_scene_far;
            constants.far_scene_near = _fog_params.far_scene_near;
            constants.far_scene_far = _fog_params.far_scene_far;
            ...
            // Load atmosphere cubemap if needed
            ID3D11ShaderResourceView* atmosphere_srv = nullptr;
            if (_fog_params.atmosphere_enabled && !_fog_params.atmosphere_texture_name.empty()) {
                atmosphere_srv = textures.at_if(_fog_params.atmosphere_texture_name).get();
            }

            // Load space cubemap if needed
            ID3D11ShaderResourceView* space_srv = nullptr;
            if (!_fog_params.space_texture_name.empty()) {
                space_srv = textures.at_if(_fog_params.space_texture_name).get();
            }

            // Bind textures: scene, depth_near, depth_far, noise, atmosphere_cubemap, space_cubemap
        ...
        struct Fog_constants {
            ...
            glm::mat4 inv_view_proj_far;  // 64 bytes (for far scene depth linearization)

            glm::vec3 fog_color;          // 12 bytes
            ...
            float cubemap_mip_scale;      // 4 bytes
            ...
            float atmos_intensity;        // 4 bytes (BF3: atmosdata.x)
            float atmos_falloff;          // 4 bytes
            float horizon_offset;         // 4 bytes
            float _pad_atmos;             // 4 bytes padding

            // BF3 height blending
            float atmos_high_intensity;   // 4 bytes (BF3: atmosdata.z)
            float height_blend_weight;    // 4 bytes (BF3: atmosdata.w)
            glm::vec2 _pad1;              // 8 bytes padding

            // Cubemap alignment transform (HLSL float3x3 is stored as 3 rows of float4)
            glm::vec4 cubemap_rotation_row0;  // 16 bytes (x,y,z + padding)
            ...
            // Space cubemap blending (BF3 skyblend style)
            float altitude_blend_start;   // 4 bytes
            float altitude_blend_end;     // 4 bytes
            float sky_blend_override;     // 4 bytes
            int use_space_cubemap;        // 4 bytes
            glm::vec2 _pad3;              // 8 bytes padding

            // Debug: depth linearization visualization
            int debug_depth_enabled;      // 4 bytes
            int debug_buffer_mode;        // 4 bytes (0=min, 1=near only, 2=far only, 3=show which)
            float debug_max_distance;     // 4 bytes
            float near_scene_near;        // 4 bytes
            float near_scene_far;         // 4 bytes
            float far_scene_near;         // 4 bytes
            float far_scene_far;          // 4 bytes
            float _pad_debug;             // 4 bytes padding
        };

        static_assert(sizeof(Fog_constants) == 432);  // 400 + 32 bytes for debug params

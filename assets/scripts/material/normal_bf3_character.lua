...
function make_constant_buffer(props, resources_desc_view)
   ...
   cb:set("use_reflection", props:get_bool("UseReflection", false))
   cb:set("reflection_fresnel_power", props:get_float("ReflectionFresnelPower", 1.5))
   cb:set("reflection_intensity", props:get_float("ReflectionIntensity", 1.0))
   cb:set("reflection_lum_power", props:get_float("ReflectionLumPower", 1.0))
...
function get_shader_flags(props, flags)
   if props:get_bool("UseSpecularMap", false) then
      flags:add("NORMAL_BF3_CHARACTER_USE_SPECULAR_MAP")
   end

   if props:get_bool("IsDynamic", false) then
      flags:add("NORMAL_BF3_CHARACTER_USE_DYNAMIC_TANGENTS")
   end

   if props:get_bool("UseParallaxMapping", false) then
      flags:add("NORMAL_BF3_CHARACTER_USE_PARALLAX_MAPPING")
   end

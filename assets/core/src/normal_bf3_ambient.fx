...
#include "normal_bf3_ambient_common.hlsl"
...
TextureCube<float3> atmosphere_map : register(ps, t14);
...
const static bool use_texcoords_transform = NORMAL_BF3_AMBIENT_USE_TEXCOORDS_TRANSFORM;
const static bool use_specular_map = NORMAL_BF3_AMBIENT_USE_SPECULAR_MAP;
const static bool use_transparency = NORMAL_BF3_AMBIENT_USE_TRANSPARENCY;
const static bool use_hardedged_test = NORMAL_BF3_AMBIENT_USE_HARDEDGED_TEST;
const static bool use_parallax_mapping = NORMAL_BF3_AMBIENT_USE_PARALLAX_MAPPING;
...
#if !NORMAL_BF3_AMBIENT_USE_DYNAMIC_TANGENTS
...
#if NORMAL_BF3_AMBIENT_USE_HARDEDGED_TEST
...
Ps_output main_ps(Ps_input input)
{
   ...
   const float2 shadow_ao_sample = normal_bf3_ambient_use_shadow_map
   ...
   surface.vertex_color = input.material_color_fade.rgb;
   ...
   // ambient_env_master style: fresnelterm + (2.0 - fresnelterm) * fastpow4(1.0 - NdotV)
   if (use_env_map) {
      ...
      float3 specmap_sample = use_specular_map 
                              ? specular_map.Sample(aniso_wrap_sampler, texcoords)
                              : float3(1.0, 1.0, 1.0);

      float3 ecol;

      // Game uses matvalue.x > 5.1 as threshold
      if (specular_exponent > 5.1) {
         // ambient_env_master fresnel: fresnelterm + (2.0 - fresnelterm) * fastpow4(1.0 - NdotV)
         const float NdotV = saturate(dot(normalWS, viewWS));
         float t = 1.0 - NdotV;
         t *= t;  // ^2
         t *= t;  // ^4 (fastpow4)
         
         const float fresnelterm = 0.04;
         const float envfresnel = saturate(fresnelterm + (2.0 - fresnelterm) * t);
         
         // Specular luminance threshold (game's threshenv = {0.3, 1/0.7})
         const float sy = dot(specmap_sample, float3(0.299, 0.587, 0.144));
         const float spec_thresh = saturate((sy - 0.3) * (1.0 / 0.7));
         
         ecol = env * envfresnel * specmap_sample * env_map_vis * spec_thresh * 2.0;
      }
      else {
         // Fallback: dim reflection
         ecol = 0.05 * env;
      }

      color += (gloss * ecol * base_specular_color);
   }

   // Apply fog.
   color = apply_fog(color, input.fog);

   // Calculate terrain fade - color only, no transparency
   if (use_terrain_fade) {
      // Distance from world origin (0,0,0) - horizontal only
      float dist = length(input.positionWS.xz);
      
      // Fade factor for color effects (FadeStart to FadeEnd)
      float fade_range = max(fade_end - fade_start, 1.0);
      float fade_factor = saturate((dist - fade_start) / fade_range);
      fade_factor = pow(fade_factor, fade_power);
      
      // Desaturate distant terrain
      float lum = dot(color, float3(0.299, 0.587, 0.114));
      color = lerp(color, float3(lum, lum, lum), fade_factor * horizon_desaturation);
      
      // Get fade color - either from cubemap or flat color
      float3 fade_color = horizon_color;
      if (use_atmosphere_map) {
         // Sample cubemap using view direction to this pixel
         float3 view_dir = normalize(input.positionWS - view_positionWS);
         
         // Apply flip adjustments to match skybox orientation
         view_dir.x *= atmosphere_flip_x;
         view_dir.y *= atmosphere_flip_y;
         view_dir.z *= atmosphere_flip_z;
         
         // DEBUG: Uncomment to visualize sample direction as RGB
         // R = +X (right), G = +Y (up), B = +Z (forward)
         // color = view_dir * 0.5 + 0.5; // Shows direction as color
         
         fade_color = atmosphere_map.Sample(aniso_wrap_sampler, view_dir);
      }
      
      // Blend toward fade color with intensity control
      color = lerp(color, fade_color * horizon_intensity, fade_factor);
   }
   ...
   // Apply terrain fade to alpha
   // Apply terrain fade to alpha (disabled - using opaque color fade)
   // alpha *= terrain_fade_alpha;

...
#include "normal_bf3_character_common.hlsl"
...
TextureCube<float3> reflection_map : register(ps, t14);
...
const static bool use_texcoords_transform = NORMAL_BF3_CHARACTER_USE_TEXCOORDS_TRANSFORM;
const static bool use_specular_map = NORMAL_BF3_CHARACTER_USE_SPECULAR_MAP;
const static bool use_transparency = NORMAL_BF3_CHARACTER_USE_TRANSPARENCY;
const static bool use_hardedged_test = NORMAL_BF3_CHARACTER_USE_HARDEDGED_TEST;
const static bool use_parallax_mapping = NORMAL_BF3_CHARACTER_USE_PARALLAX_MAPPING;
...
#if !NORMAL_BF3_CHARACTER_USE_DYNAMIC_TANGENTS
...
#if NORMAL_BF3_CHARACTER_USE_HARDEDGED_TEST
...
Ps_output main_ps(Ps_input input)
{
   ...
   const float2 shadow_ao_sample = normal_bf3_character_use_shadow_map
   ...
   // Character shader: ambient_lookup = cmap * env.rgb * intensityinfo.x
   // Sampled with NORMAL direction (ambient IBL style), no fresnel
   if (use_env_map) {
      const float3 env = env_map.Sample(aniso_wrap_sampler, normalWS);
      
      // Game: ambient_lookup = cmap * env.rgb * intensityinfo.x
      // Tint env by diffuse color, no fresnel applied
      color += env_map_vis * env * diffuse_color;
   }

   // Apply separate reflection cubemap, if using.
   // Character shader: reffresnel = 1 - pow(NdotV, fresnelfalloff)
   //                   ref = texCUBE(refmap, reflect(-cameradir, normal))
   //                   ref *= pow(luminance, intensityinfo.z) * intensityinfo.y
   //                   reflection_calc = smap * ref * specinfo.rgb * reffresnel
   if (use_reflection) {
      const float3 reflection_vec = reflect(-viewWS, normalWS);
      float3 ref = reflection_map.Sample(aniso_wrap_sampler, reflection_vec);
      
      // Reflection fresnel
      const float NdotV = saturate(dot(normalWS, viewWS));
      const float reffresnel = 1.0 - pow(NdotV, reflection_fresnel_power);
      
      // Intensity boost based on reflection luminance
      const float ref_luminance = (ref.x + ref.y + ref.z) / 3.0;
      ref *= pow(ref_luminance, reflection_lum_power) * reflection_intensity;
      
      // Specular map masking
      float3 smap = use_specular_map 
                    ? specular_map.Sample(aniso_wrap_sampler, texcoords)
                    : float3(1.0, 1.0, 1.0);
      
      color += smap * ref * base_specular_color * reffresnel;
   }
